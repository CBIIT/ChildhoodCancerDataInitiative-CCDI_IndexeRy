...
parser = argparse.ArgumentParser(
                    prog='CCDI_IndexeRy.py',
                    description='This script will take a CCDI metadata manifest file and converts to a CDS-like Indexing template based on a fixed set of property equivalencies.',
                    )
...
print('\nThe CCDI to Index conversion has begun.\n\n')
...
simple_add('experimental_strategy_and_data_subtype','experimental_strategy_and_data_subtype')
...
if 'diagnosis_icd_o' in df_join_all.columns:
    index_df['primary_diagnosis']=df_join_all['diagnosis_icd_o'].str.split(":").str[1].str.strip()
elif 'diagnosis_classification' in df_join_all.columns:
    index_df['primary_diagnosis']=df_join_all['diagnosis_classification'].str.split(":").str[1].str.strip()
else:
...
file_returned=len(index_df.loc[:,['md5sum','file_name','file_url_in_cds']].drop_duplicates())
...
print("\nWriting out the Index TSV file.\n")
